...
fn test_send_taker_fee() {
    ...
    let result = coin.validate_fee(&tx, &[], &DEX_FEE_ADDR_RAW_PUBKEY, &amount, 0).wait();
    assert_eq!(result, Ok(()));
}
...
fn test_validate_fee() {
    ...
    let err = coin
        .validate_fee(&tx, &[], &fee_addr_dif, &amount, 0)
        .wait()
        .err()
    ...
    let err = coin
        .validate_fee(&tx, &[], &DEX_FEE_ADDR_RAW_PUBKEY, &amount_dif, 0)
        .wait()
        .err()
        .expect("Expected an error");
    log!("error: "[err]);
    ...
    let err = coin
        .validate_fee(&tx, &[], &DEX_FEE_ADDR_RAW_PUBKEY, &amount, 0)
        .wait()
        .err()
        .expect("Expected an error");
    log!("error: "[err]);
